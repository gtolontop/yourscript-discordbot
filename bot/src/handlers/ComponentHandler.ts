...
async function loadButtons(client: Bot, basePath: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    if (button?.customId) {
      const id =
        button.customId instanceof RegExp
          ? button.customId.source
          : button.customId;
      client.buttons.set(id, button);
      logger.debug(`Loaded button: ${id}`);
    }
  }
...
async function loadSelectMenus(
  ...
  for (const file of files) {
    ...
    if (selectMenu?.customId) {
      const id =
        selectMenu.customId instanceof RegExp
          ? selectMenu.customId.source
          : selectMenu.customId;
      client.selectMenus.set(id, selectMenu);
      logger.debug(`Loaded select menu: ${id}`);
    }
  }
